...
{
    private const int Port = 5000; // Ensure the server is running on this port before running tests
    ...
    private static TcpClient Connect()
    {
        ...
        {
            throw new InvalidOperationException($"Unable to connect to the server on port {Port}. Ensure the server is running.", ex);
        }
        return client;
    }
}
